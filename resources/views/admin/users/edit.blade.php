        ...
        <form role="form" method="post" action="{{ route('admin.users.update', $user->id) }}" enctype="multipart/form-data">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <input type="text" value="{{ $user->name }}" class="form-control @error('nameUser') is-invalid @enderror" name="nameUser" id="nameUser" placeholder="Enter name">
                    @error('nameUser')
                ...
                <div class="mb-3">
                    ...
                    <input type="password" value="" class="form-control @error('pass') is-invalid @enderror" name="pass"
                ...
                <div class="mb-3">
                    ...
                    <select name="role" class="form-control @error('role') is-invalid @enderror">
                        ...
                        @foreach($roles as $item)
                            <option value="{{ $item->name }}" {{ $roleName == $item->name ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('role')
                    <span class="invalid-feedback" role="alert">
